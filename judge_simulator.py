...
# Config is read from a local .env file (next to this script) and/or real env vars.
# Precedence: real env var > .env > default below.
import os as _os
from pathlib import Path as _Path


def _load_dotenv() -> None:
    """Minimal .env loader (KEY=VALUE lines, '#' comments). Env vars win."""
...
# Your API key — put it in .env (GROQ_API_KEY / OPENAI_API_KEY / ...) or export it as an env var.
LLM_API_KEY = _os.environ.get(f"{LLM_PROVIDER.upper()}_API_KEY") or _os.environ.get("LLM_API_KEY") or ""
...
class BotClient:
    ...
    def push_context(self, scope, cid, version, payload):
        return self._request("POST", "/v1/context", 10, {
            ...
            "payload": payload, "delivered_at": _utc_iso()
        })

    def tick(self, triggers):
        return self._request("POST", "/v1/tick", 15, {
            "now": _utc_iso(), "available_triggers": triggers
        })
    ...
    def reply(self, conv_id, merchant_id, message, turn):
        return self._request("POST", "/v1/reply", 15, {
            ...
            "received_at": _utc_iso(), "turn_number": turn
        })
...
class JudgeSimulator:
    ...
    def _warmup(self) -> bool:
        ...
        # Best-effort state reset so repeated local runs don't trip stale_version.
        ...
        def push_line(label: str, result):
            data, err, _ = result
            ok = not err and isinstance(data, dict) and data.get("accepted")
            if err:
                detail = str(err)
            elif ok:
                detail = ""
            else:
                detail = f"reason={data.get('reason', 'unknown') if isinstance(data, dict) else 'non-JSON response'}"
            print(f"  [{'PASS' if ok else 'FAIL'}] {label}" + (f" ({detail})" if detail else ""))

        for slug, cat in self.dataset.categories.items():
            push_line(f"category/{slug}", self.client.push_context("category", slug, 1, cat))

        for mid, m in list(self.dataset.merchants.items())[:5]:
            short_id = mid.split('_')[1] if '_' in mid else mid[:10]
            push_line(f"merchant/{short_id}", self.client.push_context("merchant", mid, 1, m))
